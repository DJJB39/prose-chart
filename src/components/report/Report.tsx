  ...
  return (
    <article ref={ref} className="bg-paper text-ink">
      ...
      >
        ...
        >
          {prepared.kpis.map((k) => (
            <KpiCard key={k.label} label={k.label} value={k.value} format={k.format} trend={k.trend} />
          ))}
        </div>
      </section>
      ...
      {prepared.sections.map((section, i) => {
        ...
        if (c.type === "line") {
          body = <LineChart data={c.data} yFormat={c.yFormat} xTickFormatter={c.xIsMonth ? formatMonth : undefined} />;
        } else if (c.type === "area") {
          body = <AreaChart data={c.data} yFormat={c.yFormat} xTickFormatter={c.xIsMonth ? formatMonth : undefined} />;
        } else if (c.type === "bar") {
          ...
          body = (
            <StackedBar
              ...
              colors={colors}
              yFormat={c.yFormat}
              xTickFormatter={c.xIsMonth ? formatMonth : undefined}
            />
          );
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <span className="tabular">
            {prepared.rowCount.toLocaleString("en-GB")} rows · {formatValue("compact", prepared.rowCount)}
          </span>
        </div>
      </footer>
    </article>
